...
class Docker:
    ...
    @function
    async def publish(
        self, address: str, platform_variants: tuple[dagger.Container, ...] | None = None
    ) -> str:
        """Publish container"""
        if not platform_variants:
            platform_variants = self.platform_variants
        digest = await dag.container().publish(address=address, platform_variants=platform_variants)
        self.digest = digest
        return digest
    ...
    async def release(
        ...
        for dockerfile in dockerfiles:
            ...
            digest = await self.publish(address=f"{registry}/{repository}:latest")
            if sign:
                await self.sign(
                    digest=digest,
